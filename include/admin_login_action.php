...
$sql = "SELECT 'dentist' AS position, id, email,firstName,lastName, password,isAdmin FROM dentist WHERE email='$userEmail' 
        UNION 
        SELECT 'nurse' AS position, id, email,firstName,lastName ,password,isAdmin FROM nurse WHERE email='$userEmail' LIMIT 1";
